...
def test_no_r1_bundle_exceeds_five_files(manifest, spec):
    ...
    Currently false for sixteen R1 requirements.
    See `spec_defects.OVERSIZED_BUNDLES`.
    """
    found = bundle_mod.oversized(manifest, spec)
    assert found == spec_defects.OVERSIZED_BUNDLES, (
        "oversized bundles changed; split the requirement or update the ledger.\n"
        f"found: {found}"
    )
